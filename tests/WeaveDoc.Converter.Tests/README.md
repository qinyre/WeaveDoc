...
WeaveDoc.Converter 的单元测试和集成测试项目，覆盖 AFD 模板解析、样式映射、Pandoc 管道、配置管理和 BibTeX 解析。
...
```
WeaveDoc.Converter.Tests/
├── AfdParserTests.cs          # AfdParser 解析与验证测试
├── AfdStyleMapperTests.cs     # AfdStyleMapper 双向映射测试
├── PandocPipelineTests.cs     # Pandoc 管道集成测试
├── ConfigManagerTests.cs      # ConfigManager CRUD + 种子模板测试
├── BibtexParserTests.cs       # BibtexParser 解析测试
...
| 测试类 | 数量 | 类型 | 说明 |
|--------|------|------|------|
| AfdParserTests | 13 | 单元测试 | JSON 解析、文件解析、模板验证、异常处理、课程/实验报告模板 |
| AfdStyleMapperTests | 13 | 单元测试 | AFD↔OpenXML 双向映射（`[InlineData]` 参数化） |
| PandocPipelineTests | 17 | 集成测试 | Pandoc CLI 调用、reference.docx 生成、OpenXML 样式修正、DOCX→PDF、DCE 端到端、课程/实验报告模板 |
...
| `ReferenceDocBuilder_Build_CreatesValidDocx` | AFD 模板 → reference.docx（含 Normal 样式验证） |
| `OpenXmlStyleCorrector_ApplyAfdStyles_ModifiesDocx` | 样式修正写入 Docx |
| `OpenXmlStyleCorrector_ApplyAfdStyles_StylesTableCellParagraphs` | 表格内段落样式被修正 |
...
| `DocumentConversionEngine_ConvertAsync_Docx` | DCE Docx 端到端转换 |
...
| `FromDocxToPdfAsync_WithDocxInput_ProducesPdf` | DOCX → PDF 生成有效文件 |
| `DocumentConversionEngine_ConvertAsync_Pdf` | DCE PDF 端到端 |

#### ConfigManagerTests（8 个）

使用临时目录 + SQLite 隔离，每个测试独立：
