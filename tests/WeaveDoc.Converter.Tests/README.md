...
├── PandocPipelineTests.cs     # Pandoc 管道集成测试（15 个）
...
| 测试类 | 数量 | 类型 | 说明 |
|--------|------|------|------|
| AfdParserTests | 11 | 单元测试 | JSON 解析、文件解析、模板验证、异常处理 |
| AfdStyleMapperTests | 14 | 单元测试 | AFD↔OpenXML 双向映射（`[InlineData]` 参数化） |
| PandocPipelineTests | 15 | 集成测试 | Pandoc CLI 调用、reference.docx 生成、OpenXML 样式修正（含表格段落）、DOCX→PDF、页眉页脚、DCE 端到端 |
...
| `OpenXmlStyleCorrector_ApplyAfdStyles_StylesTableCellParagraphs` | 表格内段落样式被修正（`Descendants` 遍历） |
...
| `OpenXmlStyleCorrector_ApplyHeaderFooter` | 页眉页脚正确设置（含字体验证） |
| `OpenXmlStyleCorrector_ApplyHeaderFooter_StartPage` | 页码起始值正确设置 |
| `FromDocxToPdfAsync_WithDocxInput_ProducesPdf` | DOCX → PDF 生成有效文件（参数化字体） |
| `DocumentConversionEngine_ConvertAsync_Pdf` | DCE PDF 端到端（从修正后 DOCX 生成） |
