...
- **xUnit 2** — 测试框架
- **Avalonia.Headless 11.x** — 无头 UI 测试基础设施
- **Avalonia.Headless.XUnit 11.x** — `[AvaloniaFact]` 属性集成 xUnit
- **Avalonia.Themes.Fluent 11.x** — Fluent 主题（测试环境）
...
| `TemplateTab_SeedButton_SeedTemplates` | 执行种子模板后，DataGrid 包含至少 3 个内置模板（课程报告、实验报告、默认学术论文） |
...
| 包 | 版本 | 用途 |
|----|------|------|
| Avalonia | 11.* | UI 框架（控件实例化） |
| Avalonia.Headless | 11.* | 无头渲染平台 |
| Avalonia.Headless.XUnit | 11.* | xUnit 集成 + `[AvaloniaFact]` |
| Avalonia.Themes.Fluent | 11.* | Fluent 主题（测试环境样式） |
| Avalonia.Fonts.Inter | 11.* | Inter 字体 |
| Avalonia.Controls.DataGrid | 11.* | DataGrid 控件 |
| WeaveDoc.Converter.Ui | — | 被测 UI 项目 |
