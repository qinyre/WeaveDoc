...
```
Markdown ──→ [Pandoc + reference.docx] ──→ raw.docx ──→ [OpenXML 样式精修] ──→ Docx/PDF
                  ↑                              ↑
            AFD 模板生成                  AFD 模板样式应用
          (ReferenceDocBuilder)        (OpenXmlStyleCorrector)
```
...
- **PandocPipeline** — Pandoc CLI 封装，支持 Markdown→Docx、Markdown→PDF（XeLaTeX 中文）、AST JSON 导出
- **ReferenceDocBuilder** — 将 AFD 模板转换为 Pandoc 可用的 `reference.docx` 样式参考文件
- **OpenXmlStyleCorrector** — 对 Pandoc 输出的 Docx 进行 OpenXML 级别的精确修正：字体/字号/行距、页边距/页面尺寸、页眉页脚

### 本地配置管理 (`Config/`)

管理模板库的持久化存储和文献引用解析。

- **ConfigManager** — 公共 API，提供模板的保存、获取、列表、删除操作。编排 SQLite 元信息 + JSON 文件 + AfdParser
...
// 保存模板
var template = AfdParser.Parse("my-template.json");
await config.SaveTemplateAsync("thesis", template);

// 转换文档
var result = await engine.ConvertAsync("input.md", "thesis", "docx");
